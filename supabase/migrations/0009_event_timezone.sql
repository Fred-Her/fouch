-- FOUCH 0.3A.1 â€” event timezone for unambiguous lock-time display.
--
-- Why: prediction_lock_at is correctly an absolute instant
-- (timestamptz), but displaying it as a naked local time ("9:00 PM")
-- is ambiguous for a worldwide audience â€” a viewer in Chile, Spain, or
...
-- widen prediction_lock_at itself â€” that column remains the sole
...
comment on column events.timezone is
  ...
  'event''s canonical local time â€” display-only. Never store an '
