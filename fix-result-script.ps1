# Fix: scripts/set-official-result.ts (server-only import bug + .env.local loading).
# Run from the project root: powershell -ExecutionPolicy Bypass -File fix-result-script.ps1
try {
...
'@
    ...
    Write-Host "OK: scripts\set-official-result.ts fixed." -ForegroundColor Green
} catch {
